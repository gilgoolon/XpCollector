...
xp_collector::PopupHandler::PopupHandler(std::string client_id)
    : ICommandHandler(client_id)
{
}

std::unique_ptr<IRequest> xp_collector::PopupHandler::handle(std::shared_ptr<BasicCommand>& command)
{
